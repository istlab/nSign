    ...
    else
    {
        for (; !i.done(); ++i) 
        {
            ...
            if (fp->isScriptFrame())
            {
                ...
                ss << "(" << scriptUri.getAuthority() << ") "; /* << "@" << (unsigned)fp->script()->lineno << ") "; */
            }
            if (fp->isFunctionFrame()) 
            {
                ...
                else if (v.isObject() && v.toObject().isFunction()) 
                {
                    ...
                    if(!fun->atom)
                        ...
                        ss << " " << JS_GetStringBytes(ATOM_TO_STRING(fun->atom));
                }
...
obj_eval(JSContext *cx, uintN argc, Value *vp)
{
    ...
    if(cx->hasfp())
    {
        /* js_DumpStackFrame(cx, cx->fp()); */
        evalStackTrace = GetEvalStackTrace(cx, cx->fp());
        /*(*fout) << "Eval Stack Trace:" << evalStackTrace << "\n"; */
    }
    sw.StopStackTrace();
    sw.StartScript();
    /* /mod */
    ...
    if (length > 2 && chars[0] == '(' && chars[length-1] == ')') {
        ...
        if (ok) {
            ...
            if (ok) {
				is_json = true;
                cerr << "JSON parsed by SpiderMonkey, skipped logging" << endl;
				sw.Stop();
                return true;
			}
           /* /mod */
    ...
    if (principals && principals->codebase && 
    	(strncmp("[System Principal]", principals->codebase, 18) !=0) && 
    	(strncmp(file, "chrome://", 9) != 0))
    {
		sw.StartSig();
        JSString * decompiled = JS_DecompileScript(cx, script, file, 0);
		sw.StopSig();
		if(decompiled && !decompiled->empty())
		{
			...
			if(!JSEngine::Instance()->ValidateSignature(
                ...
                std::string(file), (int)line, evalStackTrace, &sw, hash, &is_json))
             (*fout) << "[jsobj] Signature validation failed" << std::endl;
            //return JS_FALSE;
		}
    } 
    sw.StartScript();
    ...
    *bucket = script;
    ...
    {
        (*fout) << "[jsobj]["<< id <<"] " << sw.format() << ", Length:" << srclength << endl;
        //cerr << "[jsobj][" << id << "] " << sw.format()  << ", Length:" << srclength << endl;
