...
/* mod */
static string GetJSFrameArgs(const JSStackFrame *fp )
{
	if(!fp->hasArgs())
		return std::string("");

	stringstream ss(std::stringstream::in | std::stringstream::out);
	for(Value *arg = fp->actualArgs(); arg!= fp->actualArgsEnd(); arg = arg++)
	{
		if(arg->isString())
		{
			JSString *val = arg->toString();
			ss << JS_GetStringBytes(val) << std::endl;
		}
	}

	//fprintf(stderr, "callobj");
	//(ObjectValue(* (fp->maybeCallObj())));
	//if(fp->hasArgsObj())
	//{
	//	fprintf(stderr, "argsobj");
	//	js_DumpValue(ObjectValue(* (fp->maybeArgsObj())));
	//}
	return ss.str();
}
...
RunScript(JSContext *cx, JSScript *script, JSStackFrame *fp)
{
    ...
    bool is_json = false;
    std::string code;
	std::ofstream * fout = JSEngine::Instance()->LogStream();

	if(script && script->principals && script->principals->codebase 
		&&(strncmp("[System Principal]", script->principals->codebase, 18) !=0) && 
			(strncmp(script->filename, "chrome://", 9) != 0))
	{
		sw.StartSig();
		JSString * decompiled = NULL;
		if(fp && fp->isFunctionFrame())
		{
			try
			{
				//decompiled = JS_DecompileFunction(cx, fp->fun(), 0);
				decompiled = js_DecompileToString(cx, script->filename, fp->fun(),
                                0 & ~JS_DONT_PRETTY_PRINT,
                                !(0 & JS_DONT_PRETTY_PRINT),
                                false, false, js_DecompileFunction);
			}
			catch(...)
			{
				(*fout) << "[jsinterp] Failed to decompile function script" << std::endl;
			}
		}
		if(!decompiled)
		{
			try
			{
				decompiled = JS_DecompileScript(cx, script, script->filename, 0);
			}
			catch(...)
			{
				(*fout) << "[jsinterp] Failed to decompile script" << std::endl;
			}
		}
		sw.StopSig();
		
		if(decompiled && !decompiled->empty())
		{
			const char* bytes = js_GetStringBytes(cx, decompiled);
			code.append(bytes, decompiled->length());
			srclength = code.length();
			JSPrincipals *principals = script->principals;
			
			if (!JSEngine::Instance()->ValidateSignature(
            code, 
            std::string(principals->codebase), 
            std::string(script->filename), (int)script->lineno, &sw, hash, &is_json))
	            //return  JS_FALSE;
				JSEngine::Instance()->Log(string("[jsinterp] Signature validation failed"));
		}

		sw.StartArgs();
		std::string frameArgs = GetJSFrameArgs(fp);
		if(!frameArgs.empty())
		{
			std::vector<JSUri> * urls = JSUri::extractUris(frameArgs);
			if(urls && !urls->empty())
			{
				(*fout) << "[jsinterp] Stack Frame Args:" << frameArgs << "\n";
				//std::cout <<  "[jsinterp] Stack Frame Args:" << frameArgs << "\n";
				...
				for(std::vector<JSUri>::iterator it = urls->begin(); it != urls->end(); it++)
				{
					...
					{
						JSEngine::Instance()->Log(string("[jsinterp] Invalid url detected in arguments"));
						//return JS_FALSE;
					}
    ...
    {
        std::ofstream *fout = JSEngine::Instance()->LogStream();
        (*fout) << "[jsinterp] " << sw.format() << std::endl;
        //cerr << "[jsinterp] "<< sw.format() << std::endl;
		string id("JSIN");
		JSEngine::Instance()->AddBenchResult(string(hash), id , sw);
    }
	/* /mod */
	return result;
}
