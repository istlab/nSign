                                 ...
                                 jsval *rval)
{
	...
	(strncmp(filename, "chrome://", 9) != 0))
    {
		sw.StartSig();
		JSString * decompiled = JS_DecompileScript(cx, script, filename, 0);
		sw.StopSig();
		if(decompiled && !decompiled->empty())
		{
            ...
            std::string(filename), (int)lineno, &sw, hash, &is_json))
	            ok = JS_FALSE;
		}
	}
	sw.StartScript();
    ...
    if (sw.TotalOverhead() > 0) //Leave out xul scripts
    {
        ...
        (*fout) << "[jsapi]["<< id <<"] " << sw.format() << ", Length:" << srclength << endl;
        //cerr << "[jsapi]["<< id <<"] "<< sw.format() << ", Length:" << srclength << endl;
